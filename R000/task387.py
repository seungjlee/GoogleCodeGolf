...
def a(I):
 ...
 if K>J:
  for B in range(J+1,K):
   if min(B-J,K-B)%2==0:D[G][B]=b;D[H][B]=b
 if H>G:
  for A in range(G+1,H):
   if min(A-G,H-A)%2==0:D[A][J]=b;D[A][K]=b
 return tuple(tuple(A)for A in D)
def p(g):A=tuple(tuple(A)for A in g);B=a(A);return[list(A)for A in B]