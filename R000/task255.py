R=range
def p(g):
 N='h';J,D=len(g),len(g[0]);T=[[not g[A][B]and all(not(0<=A+C<J and 0<=B+E<D)or not g[A+C][B+E]for C in(-1,0,1)for E in(-1,0,1)if C or E)for B in R(D)]for A in R(J)];K=[0]*D;E=[]
 for O in R(J):
  for P in R(D):K[P]=T[O][P]and K[P]+1
  for L in R(D):
   C=K[L]
   if not C:continue
   for B in R(L,D):
    if not K[B]:break
    C=min(C,K[B])
    for Q in R(1,C+1):E.append((O-Q+1,L,O,B,(B-L+1)*Q))
 ...
 def r(a):
  for A in R(a[0],a[2]+1):M[A][a[1]:a[3]+1]=[3]*F(a)
