	...
	for G in range(E):
		for H in range(J):
			if g[G][H]==1 and not F[G][H]:
				...
				O=m(A for(A,B)in A);P=max(A for(A,B)in A);Q=m(A for(B,A)in A);R=max(A for(B,A)in A);X=set(A);S=[g[A][B]for A in range(O,P+1)for B in range(Q,R+1)if(A,B)not in X]
				if not S:continue
				T=N(S);Y=m(T.values());Z=m(A for(A,B)in T.items()if B==Y)
