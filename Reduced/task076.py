e=enumerate;m=min;x=max;r=range;l=len
...
def occ(g,o):
 ...
 if not A:return set()
 C,E,F,G=bb(A);H,I=F-C+1,G-E+1;return{(B,C)for B in r(B-H+1)for C in r(D-I+1)if all(g[B][C]==A for(A,(B,C))in S(A,(B,C)))}
def p(I):
 ...
 for H in G:
  ...
  for(M,N)in occ(I,A):D.append(S(E,(M-K,N-L)))
