...
R=range
def p(g):
 o=False;O=True;from collections import Counter as X,deque;K,l=L(g),L(g[0]);P=[[o]*l for A in R(K)];r=[]
 for A in R(K):
  for B in R(l):
 ...
 for i in A1:
  ...
  A3={(A,D)for(A,(B,C))in N.items()for D in R(B,C+1)};A4=min(A for(A,B)in C);A5=max(A for(A,B)in C);j={B for(A,B)in C if A==A4};k={B for(A,B)in C if A==A5};A6=j if L(j)>=L(k)else k;V={(B,A)for(B,A)in A2 if A in A6};A7=min(A for(B,A)in C);A8=max(A for(B,A)in C);l={A for(A,B)in C if B==A7};m={A for(A,B)in C if B==A8};A9=m if L(m)>=L(l)else l;Q={(A,B)for(A,B)in A3 if A in A9};W=o
  if L(V)>L(Q):J=V
  elif L(Q)>L(V):J=Q;W=O
  else:J=Q;W=O
  if W:
   AA=set(C);X={}
