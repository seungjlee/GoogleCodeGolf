...
def sb(I):
 ...
 def c(G,bg):
  F,H=L(G),L(G[0]);E=[[False]*H for A in R(F)];I=[]
  for A in R(F):
   for B in R(H):
 ...
 def C(a,b):
  A,C=a;B,D=b
  if A==B:E,H=sorted((C,D));return{(A,B)for B in R(E,H+1)}
  if C==D:F,I=sorted((A,B));return{(A,C)for A in R(F,I+1)}
